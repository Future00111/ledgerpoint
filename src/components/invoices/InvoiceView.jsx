...
export default function InvoiceView({ invoice, open, onOpenChange }) {
        ...
        <div className="space-y-4 py-2">
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">{invoice.payment_terms ? `${invoice.payment_terms} days` : '—'}</p>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <div className="w-56 space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Subtotal</span><span>{gbp.format(invoice.subtotal || 0)}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">VAT Total</span><span>{gbp.format(invoice.vat_total || 0)}</span></div>
              <div className="flex justify-between font-semibold border-t pt-2"><span>Total</span><span>{gbp.format(invoice.total || 0)}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Amount Paid</span><span>{gbp.format(invoice.amount_paid || 0)}</span></div>
              <div className="flex justify-between font-semibold text-primary border-t pt-2"><span>Balance Due</span><span>{gbp.format(invoice.balance_due || 0)}</span></div>
            </div>
          </div>
