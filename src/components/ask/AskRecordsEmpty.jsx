...
import { Search, UserPlus, Truck, FileText } from 'lucide-react';

// Shown when a record search returns nothing. Explains the outcome and offers
// similar records (if any) plus quick create actions. Never an empty void.
export default function AskRecordsEmpty({ query, similar, onPickSimilar, onCreateCustomer, onCreateSupplier }) {
  return (
    <div className="px-4 sm:px-6 py-8">
      <div className="mx-auto w-10 h-10 rounded-xl bg-muted flex items-center justify-center mb-3">
        <Search className="w-4 h-4 text-muted-foreground" />
      </div>
      <p className="text-sm font-medium text-center">No matching records were found.</p>
      <p className="text-xs text-muted-foreground mt-1 text-center">
        We searched customers, suppliers, companies, invoices, bills, credit notes, bank transactions, documents and reports for “{query}”.
      </p>

      {similar && similar.length > 0 && (
        <div className="mt-4">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground mb-1.5">Similar records</p>
      ...
      <div className="mt-4 flex items-center justify-center gap-2">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg border border-border hover:bg-muted"
        >
          <Truck className="w-3.5 h-3.5" /> Create new supplier
        </button>
      </div>
    </div>
  );
