...
export default function WorkspaceEmptyState({ icon: Icon, title, description, actionLabel, onAction, actionIcon: ActionIcon }) {
  return (
    <div className="flex flex-col items-center text-center py-10 px-4">
      ...
      {actionLabel && onAction && (
        <Button variant="outline" size="sm" className="mt-4 gap-2" onClick={onAction}>
          {ActionIcon ? <ActionIcon className="w-3.5 h-3.5" /> : <Sparkles className="w-3.5 h-3.5" />} {actionLabel}
        </Button>
      )}
    </div>
  );
