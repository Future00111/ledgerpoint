...
// Collapsed statement row — slim so 6+ fit on screen.
export default function CompactRow({ transaction, onSelect }) {
  ...
  return (
    <div
      onClick={onSelect}
      className="flex items-center gap-3 px-4 h-10 border-b border-[#E5E7EB] cursor-pointer hover:bg-muted/30 transition-colors"
    >
      <span className="text-xs text-muted-foreground w-[84px] flex-shrink-0 tabular-nums">{fmtDate(t.date)}</span>
